const rootMixins = {
  ...
  methods: {
    ...
    unixTime(time) {
      const str = Number(time) === 0 ? '-' : this.$moment.unix(time).format('YYYY-MM-DD HH:mm:ss')
      return str
    },
    newUnixTime(time) {
      const str = Number(time) === 0 ? '-' : this.$moment.unix(time).format('YYYY-MM-DD HH:mm')
      return str
    },
    returnDayTime(time) {
      const str = Number(time) === 0 ? '-' : this.$moment.unix(time).format('YYYY-MM-DD')
      return str
    },
    // 获取手个月之前的日期
    twomonth() {
    ...
    // 获取三个月之前的时间
    threemonth() {
      const start_date = this.$moment(new Date()).subtract(3, 'months').format('YYYY-MM-DD 00:00:00')
      const end_date = this.$moment()
    ...
    // 校验Ip
    checkIp(ip) {
      const reg = /(2(5[0-5]{1}|[0-4]\d{1})|[0-1]?\d{1,2})(\.(2(5[0-5]{1}|[0-4]\d{1})|[0-1]?\d{1,2})){3}/
      return reg.test(ip)
    },
    // 做添加期数的限制
    beforeToday() {
